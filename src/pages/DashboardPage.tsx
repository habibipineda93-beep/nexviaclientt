...
const recentActivity = [
  ...
  { type: "sms", message: "Envío concatenado 'Facturas Enero' completado", time: "Hace 30 min", status: "success", count: "1,200 mensajes" },
  { type: "sms", message: "Envío fallido — saldo insuficiente", time: "Hace 2 horas", status: "error", count: "350 mensajes" },
  { type: "sms", message: "Campaña 'Recordatorio Pago' programada", time: "Hace 3 horas", status: "pending", count: "800 mensajes" },
];
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Resumen de tu actividad de mensajería SMS</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button className="gap-2" style={{ background: "var(--nexvia-gradient)" }} onClick={() => navigate("/sms")}>
            <Plus className="h-4 w-4" /> Nuevo Envío SMS
          </Button>
      ...
      {/* SMS Chart - Full width */}
      <Card>
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-base font-display">Mensajes SMS Enviados</CardTitle>
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <Area type="monotone" dataKey="sms" stroke="hsl(170 60% 40%)" fill="url(#gradSms)" strokeWidth={2} name="SMS" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {[
              ...
              { label: "SMS Concatenados", icon: FileText, url: "/sms", color: "bg-accent/10 text-accent" },
